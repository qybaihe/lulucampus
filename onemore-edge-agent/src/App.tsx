...
import { DEMO_LOCAL_CONTEXT } from './demoContext';
import type { CampusCredentialsPayload, LocalContextPayload } from './types';
...
const LAMP_I18N_KEYS: Record<string, string> = {
  get_local_timetable: 'tool.timetable',
  list_local_tasks: 'tool.tasks',
  propose_schedule: 'tool.schedule',
  draft_tasks: 'tool.draft',
  search_electives: 'tool.electives',
  plan_campus_action: 'tool.campus',
};
...
function AppInner() {
  ...
  const localContextRef = useRef<LocalContextPayload>(DEMO_LOCAL_CONTEXT);
  const credentialsRef = useRef<CampusCredentialsPayload | undefined>(undefined);
  const [localContextText, setLocalContextText] = useState(
    () => JSON.stringify(DEMO_LOCAL_CONTEXT, null, 2),
  );
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      localContextRef.current = JSON.parse(localContextText) as LocalContextPayload;
      const parsedCreds = JSON.parse(credentialsText) as CampusCredentialsPayload;
      credentialsRef.current =
        parsedCreds && Object.keys(parsedCreds).length > 0 ? parsedCreds : undefined;
      setContextError(null);
    } catch {
      setContextError('localContext / credentials JSON 无效');
      return;
    }
    ...
    const ctrl = sendMessageStream(text, {
      ...
      botMsgId,
      localContext: localContextRef.current,
      credentials: credentialsRef.current,
    });

    abortCtrlRef.current = ctrl;
  }, [updateBotMessage, clearBotStreaming, finishStream, refreshConversations, t, localContextText, credentialsText]);
      ...
      <div className={styles.stage}>
        ...
        <div className={styles.chatPanel}>
          ...
          <details className={styles.contextPanel}>
            <summary>客户端上下文（localContext / credentials · 不落云端）</summary>
            <p className={styles.contextHint}>
              模拟 iOS：课表与任务随请求上传；凭证仅本轮 ephemeral。云端 Agent 只编排。
            </p>
            {contextError && <p className={styles.contextError}>{contextError}</p>}
            <label className={styles.contextLabel}>localContext</label>
            <textarea
              className={styles.contextEditor}
              value={localContextText}
              onChange={(e) => setLocalContextText(e.target.value)}
              spellCheck={false}
              rows={8}
            />
            <label className={styles.contextLabel}>credentials（可空 `{}`）</label>
            <textarea
              className={styles.contextEditor}
