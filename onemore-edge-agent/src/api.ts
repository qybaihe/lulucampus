 ...
 *   agents/chat/index.ts                         → POST /chat                  Web debug chat (SSE)
 *   agents/ask/index.ts                          → POST /ask                   iOS/headless chat (SSE)
 ...
 *   cloud-functions/v1/capabilities/index.ts     → GET|POST /v1/capabilities   API discovery
 *   cloud-functions/v1/electives/search/index.ts → POST /v1/electives/search   Elective search (JSON)
 *
...
export const API = {
  ...
  electivesSearch: '/v1/electives/search',
} as const;
...
export function sendMessageStream(
  ...
  options?: {
    ...
    userMsgId?: string;
    botMsgId?: string;
    localContext?: import('./types').LocalContextPayload;
    credentials?: import('./types').CampusCredentialsPayload;
  },
): AbortController {
    ...
    try {
      ...
      const res = await fetch(API.chat, {
        ...
        body: JSON.stringify({
          ...
          // iOS owns secrets: attach per-turn only; server must not persist.
