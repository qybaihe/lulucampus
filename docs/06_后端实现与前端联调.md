...
本地种子账号使用 `X-User-ID: u_demo_1`；`u_demo_1` 至 `u_demo_6` 为中大测试剧组，均可完成端到端成局。也可用 `13900001001`–`006` / `cast-onemore` 走手机号登录。开发态可开 `ONEMORE_CAST_DRIVER_ENABLED`，让剧组按课表和性格走真实成局接口（生产必须关闭）。生产环境关闭 `ONEMORE_DEV_AUTH_ENABLED`，使用扫码成功后轮询得到的 `om1.*` 签名 Bearer Token。
