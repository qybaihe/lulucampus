...
import { LuluSprite } from "./LuluSprite";
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setStatusInfo(info);
        if (!info.enabled) {
          setPhase({ kind: "failed", message: info.message || "体验入口未开启" });
        }
      } catch (err) {
      ...
      <div className="demo-taste-shell">
        ...
        <main className="demo-taste-card">
          ...
          {phase.kind === "ready" && result ? (
            <>
              <PersonaCard ref={cardRef} result={result} profile={profile} />
              <div className="demo-taste-actions">
                ...
                <button type="button" className="om-btn ghost" onClick={onRestart}>
                  再看一个
                </button>
              </div>
            </>
          ...
          {phase.kind === "failed" ? (
            <div className="taste-stage">
              ...
              <div className="mt-5" style={{ width: "100%" }}>
                <button type="button" className="om-btn primary" onClick={onRestart}>
                  重新开始
                </button>
              </div>
            </div>
